...
if(num%2==0 || num%3 == 0) return false; //Multiples of prime numbers are not prime
for(int i = 6 ; i*i <=num ; i+=5){    
 //If number is a composite  number then it must have one mutiple greater(or equal) 
 //to sqrt(num) and one mutiple less(or equal) to sqrt(num).
 //If the num does'nt satisfy the above conditions then the number is said to a prime number
